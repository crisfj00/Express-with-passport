...
const strong_kdParams = {derivedKeyLength: 64, scryptParams: { logN: 18, r: 8, p: 2 }};
        ...
        if(error){
          ...
          if( res.length > 0 ) {
		    ...
		    Object.keys(res).forEach(function(key) {
		      var row = res[key];   
		      console.time("LoginKeyDev");
		      const cmp = scryptMcf.verify(password, row.password); 	
		      console.timeEnd("LoginKeyDev");
		    if (cmp) {
		      const user = { 
			username: username,
...
app.post("/register", (req, res) => {    
    ...
    db.query('SELECT name FROM users WHERE name = ?', [username], async (error, result) => {
        ...
        console.time("RegistrationKeyDev");
        let hashedPassword = await scryptMcf.hash(password,strong_kdParams)
      	console.timeEnd("RegistrationKeyDev");
